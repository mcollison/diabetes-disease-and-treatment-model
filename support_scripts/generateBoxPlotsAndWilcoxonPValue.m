function [pvalue, string_answers] = generateBoxPlotsAndPValue(variableIndex, mcategory, metaboliteName, logswitch)
figure
l=1;
...
for i=1:length(variableIndex)
    ...
    if logswitch(i)==0
        ...
        [hvalue(i), pvalue(i)]=ranksum(mcategory{1,2}{variableIndex(i),2},mcategory{2,2}{variableIndex(i),2});
    else
        ...
        set(gca,'XTickLabel', groupNames);
        title(metaboliteName(variableIndex(i)));
        [hvalue(i), pvalue(i)]=ranksum(log(mcategory{1,2}{variableIndex(i),2}),log(mcategory{2,2}{variableIndex(i),2}));
    end
    ...
    string_answers{l}=strcat('Wilcoxon rank sum test p-value for', {' '}, metaboliteName(variableIndex(i)), {' '}, 'between the', {' '}, mcategory{1,1}, {' '}, 'group and the', {' '}, mcategory{2,1}, {' '}, 'group: ', {' '}, num2str(pvalue(i)));
    l=l+1;
end